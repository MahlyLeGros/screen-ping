import { app, nativeImage, systemPreferences, nativeTheme } from "electron";
import fs from "fs";
import path from "path";
import { execSync } from "child_process";

/** Absolute path to a file under desktop/assets (dev + packaged). */
export function getAssetPath(...parts: string[]): string {
  const candidates: string[] = [];

  if (app.isPackaged) {
    candidates.push(path.join(process.resourcesPath, "assets", ...parts));
    candidates.push(path.join(process.resourcesPath, "app.asar.unpacked", "assets", ...parts));
...
export function loadAppIcon(): Electron.NativeImage {
  for (const name of ["icon.ico", "icon.png", "icon-256.png"]) {
...
// silence unused import in some electron versions
